...
print(Fore.RED + "версия r1.4-linux on windows")
...
for i in range(1,100):
    ...
    # импорт
        ...
        print('blot - блокнот')
        print('my pyw - мой файл')
        print('my py - мой файл')
        pack = input('Название пакета: ')
        if pack == 'my pyw':
            ans = "имя файла: "
            anb = 'ссылка(только с .pyw): '
            save_from_site(input(anb),f"{input(ans)}.pyw")
        elif pack == 'my py':
            ans = "имя файла: "
            anb = 'ссылка(только с .py): '
            save_from_site(input(anb),f"{input(ans)}.py")
        elif pack == 'blot':
